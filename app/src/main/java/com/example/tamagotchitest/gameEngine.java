...
class gameEngine implements tamagotchObject.TamagotchState {
    ...
    private GameOver kuuntelija = null;

    public interface GameOver{
        void gameOverMessage(String status);
        void send(Object status);
    ...
    class Tulostus extends Thread {
        ...
        public void run() {
            while (gameOver == false) {
                ...
                for (int i = 0; i < Objects.size(); i++) {
                        //kuuntelija.send(Objects.get(i));
                        kuuntelija.gameOverMessage(Objects.get(i).toString());
                   // System.out.println(Objects.get(i));
                }
